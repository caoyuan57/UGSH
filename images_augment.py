...
def get_embeddings(model, dataloader, device):
    ...
    with torch.no_grad():  # no need to call Tensor.backward(), saves memory
        ...
        for idx, x, _ in tqdm(dataloader, desc='Getting Embeddings (batches): '):
            x = x.to(device)
            feature_extractor = create_feature_extractor(model, return_nodes=['getitem_5'])
            batch_outputs.append(feature_extractor(x)['getitem_5'])
...
def get_vit_model_for_embedding(model=None):
    """
    Remove the last layer to get embeddings
...
if __name__ == '__main__':
    ...
    vitNet = get_vit_model_for_embedding()
    embeddings = get_embeddings(vitNet, images_dataloader, device)
