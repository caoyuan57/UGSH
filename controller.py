...
class Controller:
    ...
    def get_model(self):
        ...
        """
        return UGSH(self.cfg.image_dim, self.cfg.text_dim, self.cfg.hidden_dim, self.cfg.hash_dim)
    ...
    def get_optimizers(self):
        ...
        """
        # 创建一个 optimizer_gen 只包含不需要更新的模块以外的参数
        params_to_update = []
        for name, param in self.model.named_parameters():
            if 'hash_dis' not in name:  # 排除名字中包含 'hash_dis' 的参数
                params_to_update.append(param)
        optimizer_gen = Adam(params_to_update, lr=self.cfg.lr, weight_decay=0.0005)
    ...
    def train_epoch(self, epoch):
        ...
        e_losses_dict = {'bb': 0., 'quant': 0., 'ntxent': 0., 'sum': 0.}
        ...
        for i, (dataloader_idx, sample_idxs, img1, img2, txt1, txt2, label) in enumerate(self.dataloader_quadruplets):
            ...
            h_img1, h_img2, h_txt1, h_txt2, S_img1,S_img2, S_txt1, S_txt2 = self.model(img1, img2, txt1, txt2)
            ...
            e_losses_dict = self.train_hash_network(h_img1, h_img2, h_txt1, h_txt2, S_img1, S_img2, S_txt1, S_txt2, dataloader_idx, e_losses_dict)
        ...
        s = '[{}/{}] Train: {:.3f}s, Losses: BB: {bb:10.2f}, SQ: {quant:10.2f}, MCT: {ntxent:10.2f}'
        self.logger.info(s.format(epoch + 1, self.cfg.max_epoch, delta_t, **e_losses_dict))

    def train_hash_network(self, h_img1, h_img2, h_txt1, h_txt2, S_img1, S_img2, S_txt1, S_txt2, ind, e_losses_dict):
        """
        ...
        """
        err, batch_losses_dict = self.calculate_losses(h_img1, h_img2, h_txt1, h_txt2, S_img1, S_img2, S_txt1, S_txt2, ind)
    ...
    def calculate_losses(self, h_img1, h_img2, h_txt1, h_txt2, S_img1, S_img2, S_txt1, S_txt2, ind):
        """
        Calculate losses
        :param: h_img1: batch of image hashes #1 (original)
        :param: h_img2: batch of image hashes #2 (augmented)
        ...
        loss_quant = self.calc_quantization_loss(h_img1, h_img2, h_txt1, h_txt2, S_img1, S_img2, S_txt1, S_txt2, ind) * self.cfg.beta

        loss_bb = self.calc_bit_balance_loss(h_img1, h_img2, h_txt1, h_txt2) * self.cfg.gamma

        err = loss_ntxent + loss_quant + loss_bb
        e_losses = self.get_batch_losses_dict(loss_ntxent, loss_quant, loss_bb)
        return err, e_losses
    ...
    def calc_ntxent_loss(self, h_img1, h_img2, h_txt1, h_txt2):
        ...
        # loss_ntxent_augment = self.contr_loss(h_img2, h_txt2, type='orig') * self.cfg.contrastive_weights[3]
        loss_ntxent = loss_ntxent_inter + loss_ntxent_intra_txt + loss_ntxent_intra_img
        return loss_ntxent


    def calc_quantization_loss(self, h_img1, h_img2, h_txt1, h_txt2, S_img1, S_img2, S_txt1, S_txt2, ind):
        """
        ...
        MSEloss = nn.MSELoss()
        loss_quant_1 = MSEloss(h_img1 , h_txt1)
        loss_quant_2 = MSEloss(h_img1 , h_img2)
        loss_quant_3 = MSEloss(h_txt2 , h_img2)
        loss_quant_4 = MSEloss(h_txt1 , h_txt2)

        a = 0.5
        aug_S = (S_img1 * a) + (S_txt1 * (1 - a))
        # aug_S = torch.sigmoid(S_img1 +  S_img2 + S_txt1 + S_txt2) 
        # aug_S2 = a * S_img2 + (1 - a) * S_txt2

        loss_Sim_TI1 = torch.sum(torch.pow(torch.sigmoid(h_img1.matmul(h_txt1.transpose(0, 1))) - aug_S, 2))
        # loss_Sim_TI2是关键
        loss_Sim_TI2 = torch.sum(torch.pow(torch.sigmoid(h_img2.matmul(h_txt2.transpose(0, 1))) - aug_S, 2))

        # loss_Sim_TI3 = torch.sum(torch.pow(torch.sigmoid(h_txt1.matmul(h_txt2.transpose(0, 1))) - aug_S, 2))
        # loss_quant = (loss_quant_1 + loss_quant_2 + loss_quant_3  + loss_quant_4 + loss_Sim_TI1 + loss_Sim_TI2)
        loss_quant = (loss_quant_1 + loss_quant_2 + loss_quant_3 + loss_quant_4 + loss_Sim_TI1 + loss_Sim_TI2)
        # loss_quant = loss_quant_IT + loss_quant_II
        return loss_quant
    ...
    @staticmethod
    def get_batch_losses_dict(loss_ntxent, loss_quant, loss_bb):
        """
        ...
        """
        b_losses_dict = {'bb': 0., 'quant': 0., 'ntxent': 0.}
    ...
    def eval(self, epoch):
        ...
        # qBX是查询图像哈希码, qBY是查询文本哈希码
        # rBX是检索数据库图像哈希码, rBY是检索数据库图像哈希码
        # qLX 和 qLY 里的元素是相等的 rLX 和 rLY 是相等的
        qBX, qBY, rBX, rBY, qLX, qLY, rLX, rLY, indexes = self.get_codes_labels_indexes()
        # print(qBX[180:185, :])
        # print(qBY[180:185, :])
        # elementwise_equality = rLX == rLY

        # all_elements_equal = elementwise_equality.all()
        # print("Is each element equal: ")
        # print(elementwise_equality)
        # print("\nAre all elements equal: ", all_elements_equal.item())
        # pdb.set_trace()
        ...
        if mapavg > self.maps_max['avg']:
            ...
            qu_BT = list([])
            qu_L = list([])
            qu_BI.extend(qBX.cpu().data.numpy())
            qu_BI = np.array(qu_BI)
            qu_BT.extend(qBY.cpu().data.numpy())
            qu_BT = np.array(qu_BT)
            qu_L.extend(qLX.cpu().data.numpy())
            qu_L = np.array(qu_L)
            # 检索
            re_BI = list([])
            re_BT = list([])
            re_L = list([])
            re_BI.extend(rBX.cpu().data.numpy())
            re_BI = np.array(re_BI)
            re_BT.extend(rBY.cpu().data.numpy())
            re_BT = np.array(re_BT)
            re_L.extend(rLX.cpu().data.numpy())
            re_L = np.array(re_L)

            np.save("./save/"+ str(self.cfg.dataset) + "/" + str(self.cfg.hash_dim) + "-" + "tst_img_binary.npy", qu_BI)
            np.save("./save/"+ str(self.cfg.dataset) + "/"  + str(self.cfg.hash_dim) + "-" + "tst_txt_binary.npy", qu_BT)
            np.save("./save/"+ str(self.cfg.dataset) + "/"  + str(self.cfg.hash_dim) + "-" + "tst_label", qu_L)
            np.save("./save/"+ str(self.cfg.dataset) + "/"  + str(self.cfg.hash_dim) + "-" + "db_img_binary.npy", re_BI)
            np.save("./save/"+ str(self.cfg.dataset) + "/"  + str(self.cfg.hash_dim) + "-" + "db_txt_binary.npy", re_BT)
            np.save("./save/"+ str(self.cfg.dataset) + "/"  + str(self.cfg.hash_dim) + "-" + "db_label", re_L)
